...
__all__ = [
    ...
    "ensure_import", "iter_freeze", "iter_packages", "iter_packages_info", 
    "iter_uptodate_packages", "iter_outdated_packages", "iter_latest_info", 
    "update_all_packages", 
]
...
from typing import cast, Final, NamedTuple, Optional, Union
...
# NOTE: {n: p.raw_version for n, p in iter_requires('p115client')}
def iter_requires(
    name: str, 
...
) -> Iterator[tuple[str, None | Distribution]]:
    requires = {canonicalize_name(a): a for a in next(iter_packages_info([name])).requires}
    for p in get_environment(paths).iter_installed_distributions(
        ...
        if name := requires.pop(p.canonical_name, None):
            yield name, p
    yield from ((a, None) for a in requires.values())



def iter_dists(packs: Iterable[str], paths: None | list[str] = None):
    names = {canonicalize_name(a): a for a in packs}
    for p in get_environment(paths).iter_all_distributions():
        if name := names.get(p.canonical_name, None):
            yield name, p
...
def iter_packages(
    ...
    include_editables: bool = True, 
    skip: Container[str] = stdlib_pkgs, 
) -> Iterator[tuple[str, Distribution]]:
    return get_environment(paths).iter_installed_distributions(
        local_only=local_only, 
        user_only=user_only, 
        editables_only=editables_only, 
        include_editables=include_editables, 
...
def iter_packages_info(packages: Iterable[str]) -> Iterator[_PackageInfo]:
    return search_packages_info(packages)
...
def iter_uptodate_packages(
    ...
    """
    for dist in iter_packages_by_list(get_latest_version=True):
...
def update_all_packages(
    ...
    if check_outdated:
        ...
        packages = iter_packages_by_list(list_command_args)
...
def pyproject_toml_deps_update(path):
    conf = load(open(path, 'rb'))
    deps = conf["tool"]["poetry"]["dependencies"]
    requires = {k: v.lstrip(">=") for k, v in conf["tool"]["poetry"]["dependencies"].items() if k != "python" and v.startswith(">")}
    for n, p in iter_dists(requires):
        v = p.raw_version
        if v > requires[n]:
            deps[n] = ">=" + v
            print(n, requires[n], "->", v)
    new = dumps(conf)
