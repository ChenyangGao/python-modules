...
__all__ = [
    ...
    "cookie_header_for_url", "iter_resp_cookies", 
]
...
def cookie_match_domain(
    url_domain: str, 
    cookie_domain: None | str = None, 
) -> bool:
    if not cookie_domain:
        return True
    cookie_domain = cookie_domain.lstrip(".")
    return url_domain == cookie_domain or url_domain.endswith("."+cookie_domain)


def cookie_match_path(
    url_path: str, 
    cookie_path: None | str = None, 
) -> bool:
    if not cookie_path:
        return True
    cookie_path = cookie_path.rstrip("/")
    return url_path == cookie_path or url_path.startswith(cookie_path+"/")
    ...
    if expires := kwargs.get("expires"):
        if isinstance(expires, datetime):
            ...
            try:
                ...
                pass
    elif max_age := (kwargs.get("max-age") or "").strip(" ,"):
        try:
            max_age = int(max_age)
        except ValueError:
    ...
    if expires := cookie.get("expires"):
        if isinstance(expires, datetime):
            ...
            try:
                ...
                expires = None
    elif max_age := (cookie.get("max-age") or "").strip(" ,"):
        try:
            max_age = int(max_age)
        except ValueError:
...
def cookie_header_for_url(cookies, url: str, /) -> dict:
    if isinstance(cookies, CookieJar):
        req = Request(url)
        cookies.add_cookie_header(req)
        return cookies_str_to_dict(req.unredirected_hdrs.get("Cookie") or "")
    now = int(time())
    data: dict[str, str] = {}
    urlp = urlsplit(url)
    url_domain = urlp.netloc
    url_path   = urlp.path
    for name, cookie in iter_items(cookies):
        if isinstance(cookie, Cookie):
            if not cookie_match_domain(url_domain, cookie.domain):
                continue
            if not cookie_match_path(url_path, cookie.path):
                continue
            if cookie.expires and int(cookie.expires) <= now:
                continue
            cookie = cookie.value
        elif isinstance(cookie, Morsel):
            if not cookie_match_domain(url_domain, cookie.get("domain")):
                continue
            if not cookie_match_path(url_path, cookie.get("path")):
                continue
            if expires := cookie.get("expires"):
                if isinstance(expires, datetime):
                    expires = expires.timestamp()
