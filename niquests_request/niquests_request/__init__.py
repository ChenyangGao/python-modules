...
from niquests.models import Request, Response, AsyncResponse
...
if "__del__" not in AsyncResponse.__dict__:
    def close(self, /):
        from asynctools import run_async
        run_async(self.close())
    setattr(AsyncResponse, "__del__", close)
...
if "__del__" not in AsyncSession.__dict__:
    def close(self, /):
        from asynctools import run_async
        run_async(self.close())
    setattr(AsyncSession, "__del__", close)

adapters.DEFAULT_RETRIES = 5
_DEFAULT_SESSION = Session(pool_maxsize=256, pool_connections=64)
_DEFAULT_ASYNC_SESSION = AsyncSession(pool_maxsize=256, pool_connections=64)
...
def request_sync[T](
    ...
    **request_kwargs, 
    ...
    request_kwargs.update(session.merge_environment_settings(
        cast(str, prep.url), 
        **dict(get_all_items(request_kwargs, *_MERGE_SETTING_KWARGS)), 
    ))
    ...
    if parse is None:
        ...
        try:
            if (response.http_version or 0) < 20 and (
                method == "HEAD" or 
...
async def request_async[T](
    ...
    session: None | AsyncSession = _DEFAULT_ASYNC_SESSION, 
    *, 
    parse: Callable[[AsyncResponse, bytes], T], 
    **request_kwargs, 
) -> T:
    ...
async def request_async[T](
    ...
    session: None | AsyncSession = _DEFAULT_ASYNC_SESSION, 
    *, 
    parse: None | EllipsisType| bool | Callable[[AsyncResponse, bytes], T] = None, 
    **request_kwargs, 
) -> AsyncResponse | bytes | str | dict | list | int | float | bool | None | T:
    request_kwargs["allow_redirects"] = follow_redirects
    request_kwargs["stream"] = stream
    if session is None:
    ...
    if parse is None:
        ...
        return response
    content = (await response.content) or b""
    if isinstance(parse, bool):
        if not parse:
            return content
        parse = cast(Callable, parse_response)
...
def request[T](
    ...
    cookies: None | CookieJar | BaseCookie = None, 
    *, 
    parse: None | EllipsisType| bool | Callable[[AsyncResponse, bytes], T] | Callable[[AsyncResponse, bytes], Awaitable[T]] = None, 
    async_: Literal[True], 
    **request_kwargs, 
) -> Coroutine[Any, Any, AsyncResponse | bytes | str | dict | list | int | float | bool | None | T]:
    ...
def request[T](
    url: string | SupportsGeturl | URL | Request, 
    method: string = "GET", 
    ...
    cookies: None | CookieJar | BaseCookie = None, 
    *, 
    parse: None | EllipsisType| bool | Callable[[Response, bytes], T] | Callable[[AsyncResponse, bytes], T] | Callable[[AsyncResponse, bytes], Awaitable[T]] = None, 
    async_: Literal[False, True] = False, 
    **request_kwargs, 
) -> Response | bytes | str | dict | list | int | float | bool | None | T | Coroutine[Any, Any, AsyncResponse | bytes | str | dict | list | int | float | bool | None | T]:
    if async_:
